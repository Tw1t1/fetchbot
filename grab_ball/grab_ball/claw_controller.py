...
from grab_ball.hardware import ADCReader, L298N
...
class ClawController(Node):
    ...
    def __init__(self):
        ...
        self.claw_cmd_sub = self.create_subscription(
            String, 'claw_cmd', self.claw_cmd_callback, 10)
        
        self.position_pub = self.create_publisher(
            Float64, 'position', 10)

        self.timer = self.create_timer(0.1, self.timer_position_pub)
    ...
    def claw_cmd_callback(self, msg):
        try:
            ...
            if (self.current_position > self.claw_sensor_threshold) or \
                (self.current_position < 100 - self.claw_sensor_threshold):
