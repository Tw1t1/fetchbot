...
class FeelForceNode(Node):
    def __init__(self):
        ...
        self.collision_range_threshold = 0.23
        self.collision_magnitude = 100000
        
        # New parameters for asymmetric force field
        self.front_multiplier = 1.5  # Increase force for obstacles in front
        self.rear_multiplier = 1.3   # Increase force for obstacles behind
        self.side_multiplier = 0.7   # Decrease force for obstacles on the sides
    ...
    def scan_callback(self, msg):
        ...
        for i, range_value in enumerate(msg.ranges):
            if range_value < msg.range_max:
                ...
                # Determine the multiplier based on the angle
                if -math.pi/4 <= angle <= math.pi/4:  # Front
                    multiplier = self.front_multiplier
                elif math.pi*3/4 <= angle or angle <= -math.pi*3/4:  # Rear
                    multiplier = self.rear_multiplier
                else:  # Sides
                    multiplier = self.side_multiplier
                
                # Calculate force contribution
                if range_value <= self.collision_range_threshold:
